...
;FUNCION PROPIA LENGTH
(define (length lista len)
  (if (null? lista)
      len
      (length (cdr lista) (+ len 1))))
;FUNCION PROPIA REF
(define (ref lista ind cont)
  (if (not(null? lista))
      (if (= ind cont)
          (car lista)
          (ref (cdr lista) ind (+ cont 1)))
      -1))
;FUNCION PROPIA SET!
(define (myset lista pos val)
  (cond
    [(= pos 0) (cons val (cdr lista))]
    [else (cons (car lista) (myset (cdr lista) (- pos 1) val))]))
...
(define (fecha? passw)
  ...
  (if (and(integer? m)(integer? a)(integer? d)(<= d (dias m a))(< m 13))
      (begin
        (set! fecha (list (+ passw d) (+ m passw) (+ a passw)))
        fecha)
      (begin
...
(define (buscarid basedatos passw id ind)
  (define producto 0)
  (if (and (not (integer? (ref basedatos ind 0)))(< ind (length basedatos 0)))
      (begin
        (set! producto (ref basedatos ind 0))
        (if (= (- (ref producto 0 0) passw) id)
          ind
...
(define (mostrarfecha fecha passw)
  (display "\nDIA DE VENCIMIENTO: ")
  (displayln (- (ref fecha 0 0) passw))
  (display "MES DE VENCIMIENTO: ")
  (displayln (- (ref fecha 1 0) passw))
  (display "AÑO DE VENCIMIENTO: ")
  (displayln (- (ref fecha 2 0) passw)))
;FUNCIÓN PARA MOSTRAR LOS PRODUCTOS DEL TIPO QUE INDIQUE EL USUARIO.
(define (mostrarprods tipo basedatos ind passw)
  (define producto 0)
  (define nombre 0)
  (define unidad 0)
  (if (and (< ind (length basedatos 0))(not(integer? (ref basedatos ind 0))))
      (begin
        (set! producto (ref basedatos ind 0))
        (if (char-ci=? tipo (desencarac (ref producto 1 0) passw))
            (begin
              (display "ID DEL PRODUCTO: ")
              (displayln (- (ref producto 0 0) passw))
              (display "---------------------------------\n")
...
(define (mostrarprecio basedatos precio passw ind)
  ...
  (define unidad 0)  
      (if (and(< ind (length basedatos 0))(not(integer? (ref basedatos ind 0))))
          (begin
            (set! producto (ref basedatos ind 0))      
            (if (> (- (ref producto 5 0) passw) precio)
                (begin        
                  (set! nombre (ref producto 2 0))
                  (display "NOMBRE DEL PRODUCTO: ")
                  (display (desencrip nombre passw 0 (make-string (string-length nombre) #\a)))
                  ...
                  (display (- (ref producto 5 0) passw))
                  (display "\n---------------------------------\n")
                  (displayln "CANTIDAD DE UNIDADES: ")
                  (displayln (- (ref producto 6 0) passw))
                  (display "---------------------------------\n")
                  (display "VALOR TOTAL DE LOS PRODUCTOS: ")
                  (displayln (- (ref producto 7 0) passw))
...
(define (unidades basedatos unidad passw ind)
  ...
  (if (and(< ind (length basedatos 0))(not(integer? (ref basedatos ind 0))))
      (begin
        (set! producto (ref basedatos ind 0))
        (if (string-ci=? unidad (desencrip (ref producto 4 0) passw 0 (make-string (string-length (ref producto 4 0)) #\a)))
            (begin
              (set! tipo (desencarac (ref producto 1 0) passw))
              (display "ID DEL PRODUCTO: ")
              (displayln (- (ref producto 0 0) passw))
              (display "---------------------------------\n")
              (display "TIPO DE PRODUCTO: \n")
...
(define (ingresar basedatos nombre passw ind)
  ...
  (if (= (read) passw)
      (begin
        ...
        (set! basedatos (myset basedatos ind producto))
        (display basedatos);LLAMADO POR COMENTAR
        (menu basedatos nombre passw (+ ind 1)))
      (begin
       (display "La contraseña no es correcta.\nEjecutando de nuevo.")
       (ingresar basedatos nombre passw ind)
       )))
;FUNCIÓN PARA MODIFICAR EL VALOR UNITARIO Y TOTAL.
(define (valoruni passw datos)
  (define vu 0)
  (define vt 0)
  (define cu 0)
  (define datosnew 0)
       ...
       (begin
        ...
        (if (integer? vu)
            (begin
              (set! vu (+ passw vu))
              (set! vt (+ passw (* vu cu)))
              (set! datosnew (myset datos 5 vu))
              (set! datosnew (myset datosnew 6 cu))
              (set! datosnew (myset datosnew 7 vt))
              datosnew)
            (begin
  ...
  (if (integer? id)
      (begin
        ...
        (if (and(string? tipo)(esprod? (string-ref tipo 0)))
            (begin
              (set! datos (myset datos 1 (encarac (string-ref tipo 0) passw)))
              (set! datos (myset datos 0 (+ id passw)))
              datos)
            (begin
...
(define (nombreuni datos passw)
  ...
  (if (string? nombre)
      (begin        
        ...
        (if (string? medida)
            (begin
              (set! datos (myset datos 2 (encriptar nombre passw 0 (make-string (string-length nombre) #\a))))
              (set! datos (myset datos 4 (encriptar medida passw 0 (make-string (string-length medida) #\a))))
              datos)
            (begin
...
(define (llenar nombre passw ind)
  (define datos (make-list 8 0))  
  (set! datos (idtipo datos passw))
  (set! datos (nombreuni datos passw))
  (set! datos (myset datos 3 (fecha? passw)))
  (set! datos (valoruni passw datos))
  datos)
...
(define (cambiardatos basedatos nombre passw ind)
  ...
  (if (not(= ind -1))
      (begin
        ...
        (set! op (read))
        (cond
          [(= op 1)(set! datos (myset datos 3 (fecha? passw)))]
...
(define (crearsuper)
  ...
  (set! basedatos (make-list (read) 0))
